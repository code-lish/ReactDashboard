...
import { selectPackageById, useUpdatePackageMutation } from "../../features/packages/packagesApiSlice";
...
const EditPackage = () => {
    ...
    const pack = useSelector((state) => selectPackageById(state, id));
    ...
    useEffect(() => {
        setValue("name", `${pack?.name?.en || pack?.name?.fa}`)
        setValue("slug", `${pack?.slug}`)
        setValue("price", `${pack?.price?.main}`)
        pack?.price?.plus && setValue("plus-price", `${pack?.price?.plus}`)
        setValue("bandwidth", `${pack?.bandwidth?.amount}`)
        setValue("capacity", `${pack?.capacity}`)
        setValue("is-package-plus", `${pack?.isPlus}`)
        setValue("type", `${pack?.bandwidth?.type}`)
        // setValue("isNew", `${pack?.bandwidth?.type}`)
    ...
    const isPackagePlus = useWatch({ control, name: 'is-package-plus', defaultValue: pack?.isPlus })
    ...
    const onSubmitHandler = async (data) => {
        try {
            ...
            await editPackage({ id, data }).unwrap();
            await MySwal.fire({
                title: t("success"),
            ...
            <Grid item xs={12} sm={4.5}>
                ...
                >
                    ...
                    <Box sx={{ mt: "10px" }}>
                        <FormGroup>
                            <FormControlLabel
                                control={<Checkbox checked={pack?.isPlus} />}
                                label="Plus package"
                                id="is-package-plus"
